...
// ==============================================================================
// STRICT SECRET VALIDATION (Production Security)
// ==============================================================================
// Fail-fast if required secrets are missing or using insecure defaults
// This prevents accidental production deployment with weak credentials
...
function validateSecrets() {
  ...
  for (const secretName of REQUIRED_SECRETS) {
    ...
    // Check if secret uses insecure default value
  ...
  if (errors.length > 0) {
    ...
    console.error('\n📋 HOW TO FIX:');
    console.error('  1. Navigate to infra/ directory');
    console.error('  2. Run: ./generate-secrets.ps1 (Windows) or ./generate-secrets.sh (Linux)');
    console.error('  3. Review the generated .env file');
...
const JWT_SECRET = process.env.JWT_SECRET; // No fallback - validated by validateSecrets()
...
(async () => {
  const adminPassword = await bcrypt.hash(process.env.ADMIN_PASSWORD, SALT_ROUNDS); // No fallback - validated
  users.set('admin', {
    userId: 'admin',
    username: 'admin',
